...
namespace xo
{
	...
	/// multiplication
	template< typename T, typename C >
	constexpr C operator*( const T& s, const arithmetic<T, C>& a ) { return C( s * a.value ); }
	template< typename T, typename C >
	constexpr C operator*( const arithmetic<T, C>& a, const T& s ) { return C( a.value * s ); }
	template< typename T, typename C >
	constexpr C operator*( const arithmetic<T, C>& a1, const arithmetic<T, C>& a2 ) { return C( a1.value * a2.value ); }
	...
	/// absolute value
	template< typename T, typename C >
	constexpr C abs( const arithmetic<T, C>& v ) { return C( v.value < T( 0 ) ? -v.value : v.value ); }
