...
namespace xo
{
	...
	template< typename T > mat33_<T> operator*( const mat33_<T>& a, const mat33_<T>& b ) {
		return mat33_<T>(
	...
	template< typename T > vec3_<T> operator*( const mat33_<T>& m, const vec3_<T>& v ) {
		return vec3_<T>(
	...
	/// Get mat33 from quaternion
	template< typename T > mat33_<T> mat33_from_quat( const quat_<T>& q ) {
		mat33_<T> m{ no_init };
