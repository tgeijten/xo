...
namespace xo
{
	...
	shape::shape( type t, float x_radius, float y_height, float z ) : type_( t ), x_( x_radius ), y_( y_height ), z_( z )
	{}

	float shape::radius() const
	{
		xo_assert( type_ == sphere || type_ == cylinder || type_ == capsule );
		return x_;
	}

	float shape::height() const
	{
		xo_assert( type_ == cylinder || type_ == capsule );
		return y_;
	}
	...
	{
		xo_assert( type_ == box );
		return vec3f( 0.5f * x_, 0.5f * y_, 0.5f * z_ );
	}
