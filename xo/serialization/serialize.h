...
namespace xo
{
	...
	template< file_format FF > struct prop_node_serializer {
		prop_node_serializer( const prop_node& pn, error_code* ec = nullptr ) : pn( pn ), ec( ec ) {}
		const prop_node& pn;
		error_code* ec;
	};

	template< file_format FF > struct prop_node_deserializer {
		prop_node_deserializer( prop_node& pn, error_code* ec = nullptr ) : pn( pn ), ec( ec ) {}
		prop_node& pn;
		error_code* ec;
	};
	...
	XO_API prop_node load_xml( const path& filename, error_code* ec = nullptr ) { return load_file< file_format::xml >( filename, ec ); }
	XO_API void save_xml( const prop_node& pn, const path& filename, error_code* ec = nullptr ) { save_file< file_format::xml >( pn, filename, ec ); }

	/// load/save contents from prop
	XO_API prop_node load_prop( const path& filename, error_code* ec = nullptr ) { return load_file< file_format::pn >( filename, ec ); }
	XO_API prop_node parse_prop( const char* str, error_code* ec = nullptr );
	XO_API void save_prop( const prop_node& pn, const path& filename, error_code* ec = nullptr ) { save_file< file_format::pn >( pn, filename, ec ); }

	/// load contents from prop
	XO_API prop_node load_ini( const path& filename, error_code* ec )  { return load_file< file_format::ini >( filename, ec ); }
	XO_API void save_ini( const prop_node& pn, const path& filename, error_code* ec = nullptr ) { save_file< file_format::ini >( pn, filename, ec ); };
}
