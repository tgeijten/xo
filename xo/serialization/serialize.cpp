...
namespace xo
{
	...
	{
		prop_node pn;
		if ( auto s = make_serializer( filename.extension().to_string(), pn, ec ) )
			return s->load_file( filename, ec );
		else return set_error_or_throw( ec, "Unsupported file format for " + filename.to_string() ), prop_node();
	}
	...
	void save_file( const prop_node& pn, const path& filename, error_code* ec )
	{
		...
		else set_error_or_throw( ec, "Could not deduce file format for " + filename.to_string() );
	}
